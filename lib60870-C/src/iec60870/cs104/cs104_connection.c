...
sendIMessage(CS104_Connection self, Frame frame)
{
    ...
    //DEBUG_PRINT("sendIMessage for debug=========");

    int rtn;
    if(self->connectionType == 3)//串口
    {
        rtn = SerialPort_write(self->commPort, T104Frame_getBuffer(frame), 0, T104Frame_getMsgSize(frame));
    }
    else
    {
...
//    unsigned long long timestamp = Hal_getTimeInMs();
    if(self->msgsendHandler_withExplain != NULL)//,Hal_getTimeInMs()   ,timestamp
    {
        ...
        if (asdu != NULL)
        {
            ...
            //for debug
            /*if(self->cs104_frame.TI == 100)
            {
                DEBUG_PRINT("hostname_");
                DEBUG_PRINT(self->hostname);
            }*///

            CS101_ASDU_destroy(asdu);
        }
        else
            DEBUG_PRINT("DEBUG_LIB60870:(warnning)sendIMessage self->msgsendHandler_withExplain = NULL=========");

    }
    else
        DEBUG_PRINT("DEBUG_LIB60870:(warnning)sendIMessage asdu get from CS101_ASDU_createFromBuffer is NULL=========");
...
receiveMessageSocket_104P(Socket socket, uint8_t* buffer)
{
    ...
    {
        #ifdef _WIN32
            qDebug("DEBUG_LIB60870:(warnning)[receiveMessageSocket_104P]Socket_read(socket, buffer, 1) return value < 1,value is %d",readFirst);
        #else
                syslog(LOG_WARNING,"[receiveMessageSocket_104P]Socket_read(socket, buffer, 1) return value < 1,value is  = %i", readFirst);
        #endif
        return readFirst;
    }
    ...
    {
        #ifdef _WIN32
                qDebug("DEBUG_LIB60870:(warnning)[receiveMessageSocket_104P]Socket_read buffer[0] != 0x68,buffer[0] == %i",buffer[0]);
        #else
...
receiveMessageSocket(Socket socket, uint8_t* buffer)
{
    ...
    {
#ifdef _WIN32
    qDebug("DEBUG_LIB60870:(warnning)[receiveMessageSocket]Socket_read(socket, buffer, 1) return value < 1,value is %d",readFirst);
#else
        syslog(LOG_WARNING,"[receiveMessageSocket]Socket_read(socket, buffer, 1) return value < 1,value is  = %i", readFirst);
#endif
        return readFirst;
    }
...
//#endif
        ...
        if (msgSize < 7) {
#ifdef _WIN32
qDebug("DEBUG_LIB60870:(warnning)checkMessage return false,I msg too small(msgSize < 7)!");
#endif
...
#ifdef _WIN32
qDebug("DEBUG_LIB60870:(warnning)Received I frame: N(S) = %d,N(R) = %d.(while self->receiveCount = %d,not equal with frameSendSequenceNumber)",frameSendSequenceNumber,frameRecvSequenceNumber,self->receiveCount);
#endif
//            if (self->importantInfoHandler != NULL)
//                self->importantInfoHandler(self->importantInfoHandlerParameter, "checkMessage return false(I format frame),Sequence error: (Shoud) Close Connection!");//
        ...
        {
#ifdef _WIN32
qDebug("DEBUG_LIB60870:(warnning)checkSequenceNumber(self, frameRecvSequenceNumber) (Shoud) return false!");
#endif
//            if (self->importantInfoHandler != NULL)
...
//#endif
        ...
        {
#ifdef _WIN32
qDebug("DEBUG_LIB60870:(warnning)CS101_ASDU == NULL,checkMessage return false!");
#endif
//            if (self->importantInfoHandler != NULL)
...
//            DEBUG_PRINT("Rcvd TESTFR_CON");//\n
        ...
        {
#ifdef _WIN32
qDebug("DEBUG_LIB60870:(warnning)checkMessage return false(S-message),Sequence error: Close connection!");
#endif
//            if (self->importantInfoHandler != NULL)
...
#ifdef _WIN32
qDebug("DEBUG_LIB60870:(warnning)handleTimeouts,self->outstandingTestFCConMessages > 2,Close connection!");
#endif
            /* close connection */
            retVal = false;
//            if (self->importantInfoHandler != NULL)
...
#ifdef _WIN32
    qDebug("DEBUG_LIB60870:(warnning)[handleConnection]Error reading from socket,loopRunning is set false,receiveMessage bytesRec == %d,strerror(errno): %s",bytesRec,strerror(errno));
#else
        syslog(LOG_WARNING,"[handleConnection]Error reading from socket,loopRunning is set false,receiveMessage bytesRec == %d,strerror(errno): %s",bytesRec,strerror(errno));
#endif
                        /*if (self->importantInfoHandler != NULL)
                        {
                            char bytesRecstr[5];
                    ...
                    if (bytesRec > 0) {
                        ...
                        if (checkMessage(self, buffer, bytesRec) == false) {
                            ...
                            loopRunning= false;
#ifdef _WIN32
qDebug("DEBUG_LIB60870:(warnning)loopRunning is set false,checkMessage(self, buffer, bytesRec) is false!");
#endif
//                            if (self->importantInfoHandler != NULL)
//                                self->importantInfoHandler(self->importantInfoHandlerParameter, "loopRunning is set false,checkMessage(self, buffer, bytesRec) return false!");
                ...
                if (handleTimeouts(self) == false)
                {
                    loopRunning = false;
#ifdef _WIN32
qDebug("DEBUG_LIB60870:(warnning)loopRunning is set false,handleTimeouts(self) is false!");
#endif
//                    if (self->importantInfoHandler != NULL)
//                        self->importantInfoHandler(self->importantInfoHandlerParameter, "loopRunning is set false,handleTimeouts(self) return false!");
                ...
                if (self->close)
                {
                    loopRunning = false;
#ifdef _WIN32
qDebug("DEBUG_LIB60870:(warnning)loopRunning is set false,self->close is true!");
#endif
//                    if (self->importantInfoHandler != NULL)
...
#ifdef _WIN32
qDebug("DEBUG_LIB60870:(warnning)EXIT CONNECTION HANDLING THREAD!");
#endif
//    if (self->importantInfoHandler != NULL)
...
handleConnection_comm(void* parameter)
{
            ...
            if (read == 0x68||read == 0x69) {//帧头：兼容私有规约和标准104规约
                ...
                if(read == 0x69)
                {
                    ...
                    if (readBytes == (msgSize+4)) {//读取到完整帧，调用解帧函数
                        ...
                        //DEBUG_PRINT("DEBUG_LIB60870:(warnning)RECV[69]: Timeout reading variable length frame size = %i (expected = %i)\n", readBytes, msgSize);
#ifdef _WIN32
        ...
        syslog(LOG_WARNING,"RECV[69]: Timeout reading variable length frame size = %i (expected = %i)", readBytes, msgSize);
                ...
                else //if(read == 0x68)
                {
                    ...
                    if (readBytes == msgSize) {//读取到完整帧，调用解帧函数
                        ...
                        //DEBUG_PRINT("DEBUG_LIB60870:(warnning)RECV[68]: Timeout reading variable length frame size = %i (expected = %i)\n", readBytes, msgSize);
#ifdef _WIN32
    ...
    sync_error:
        ...
        DEBUG_PRINT("DEBUG_LIB60870:(warnning)RECV: SYNC ERROR\n");
        ...
        syslog(LOG_WARNING,"Error reading from socket, strerror(errno) is %s", strerror(errno));
            ...
            if (bytesRec > 0) {
                ...
                if (checkMessage(self, buffer, bytesRec) == false) {
                    /* close connection on error */
                    self->running = false;//loopRunning= false;
                    DEBUG_PRINT("DEBUG_LIB60870:(warnning)loopRunning is set false,checkMessage(self, buffer, bytesRec) is false");//\n
                    self->failure = true;
                }
...
//                }
        ...
        {
            self->running = false;//loopRunning = false;
            DEBUG_PRINT("DEBUG_LIB60870:(warnning)loopRunning is set false,handleTimeouts(self) is false\n");//
        }

        if (self->close)
        {
            self->running = false;//loopRunning = false;
            DEBUG_PRINT("DEBUG_LIB60870:(warnning)loopRunning is set false,self->close is true\n");//
        }
...
//                isAsduWaiting = sendWaitingASDUs(self);
    ...
    DEBUG_PRINT("DEBUG_LIB60870:(warnning)Connection closed\n");
...
sendASDUInternal(CS104_Connection self, Frame frame)
{
    ...
    if (self->running||self->isCommOpend) {
        ...
        {
        #ifdef _WIN32
                qDebug("DEBUG_LIB60870:(warnning)[sendASDUInternal]isSentBufferFull(self) is true");
        #else
                syslog(LOG_WARNING,"[sendASDUInternal]self->running or self->isCommOpend is false");
        #endif
                self->oldestSentASDU = -1;
                sendIMessageAndUpdateSentASDUs(self, frame);
                retVal = true;
    ...
    {
    #ifdef _WIN32
            qDebug("DEBUG_LIB60870:(warnning)[sendASDUInternal]self->running or self->isCommOpend is false");
    #else
            syslog(LOG_WARNING,"[sendASDUInternal]self->running or self->isCommOpend is false");
    #endif
    }
...
serverThread_mStation (void* parameter)
{
    ...
    if (serverSocket == NULL) {
        DEBUG_PRINT("DEBUG_LIB60870:(warnning)Cannot create server socket\n");
    ...
    while (self->stopRunning == false) {
        ...
        if (newSocket != NULL) {
            ...
            if (acceptConnection && (self->connectionRequestHandler_mStation != NULL)) {
                ...
                DEBUG_PRINT("DEBUG_LIB60870:new Connection of ipAddress = %s;p1 = %s;p2 = %s!;port = %d\n",ipAddress,p1,p2,port_new);
...
//                    //self->socket, self->hostname, self->tcpPort
                    ...
                    DEBUG_PRINT("DEBUG_LIB60870:(warnning)Connection attempt failed!");
...
//    }
    DEBUG_PRINT("DEBUG_LIB60870:CS104_Connection_setASDUReceivedHandler for the %d connection",con_index);
...
{
    DEBUG_PRINT("DEBUG_LIB60870:CS104_Connection_setMSGReceivedHandler_mStation for the %d connection",con_index);
    if(con_index < 0 || con_index > 1024)
        return ;
    if(self->connections_mStation[con_index] != NULL)
    {
...
{
    DEBUG_PRINT("DEBUG_LIB60870:CS104_Connection_setMSGSendHandler_mStation for the %d connection",con_index);
    if(con_index < 0 || con_index > 1024)
        return ;
    if(self->connections_mStation[con_index] != NULL)
    {
...
//    buffer[5] = (uint8_t) (self->receiveCount / 128);
    ...
    else
    {
        if (writeToSocket(self, buffer, msgSize) != -1) {
            DEBUG_PRINT("DEBUG_LIB60870:SEND I (size = %i) N(S) = %i N(R) = %i\n", msgSize, self->sendCount, self->receiveCount);
            self->sendCount = (self->sendCount + 1) % 32768;
            self->unconfirmedReceivedIMessages = 0;
        }
        else
...
sendASDUInternal_sDEV(CS104_Connection self, CS101_ASDU asdu)
{
    ...
    if (asduSent == false)
        DEBUG_PRINT("DEBUG_LIB60870:(warnning)unable to send response (isActive=%i)\n", self->isActive);
...
{
    DEBUG_PRINT("DEBUG_LIB60870:(warnning)  with unknown COT\n");
...
void CS104_Connection_connectDestroy(CS104_Connection self)
{
    Socket_destroy(self->socket);
    self->running = false;
}
...
void Frame_deal(CS104_Connection self, unsigned char * buf, int len)
{
    char printout[65535]={};
...
void Frame_parse(CS104_Connection self, uint8_t* buf, int len)
{
...
static void on_read(uv_stream_t* tcp, ssize_t nread, const uv_buf_t* buf)
{
    ...
    if(nread <0){
        ...
        CS104_Connection self = ((CS104_Connection)(((uv_tcp_t*)tcp)->data));
...
void on_connect(uv_connect_t* connection, int status)
{
    ...
    CS104_Connection self = ((CS104_Connection)(((uv_tcp_t*)stream)->data));
    Socket_setFd(self->socket, (int)fd_1);
    self->DEV_fd=(int)fd_1;
    syslog(LOG_INFO,"After fd get:tcpPort = %d;hostname = %s;DEV_fd =%d***firsttime link ok,sendStartDT****",((CS104_Connection)self)->tcpPort,((CS104_Connection)self)->hostname,((CS104_Connection)self)->DEV_fd);
...
bool handleTimeouts_104(CS104_Connection self)
{
...
CS104_Connection_connectAsync_uv(CS104_Connection self ,uv_loop_t* loop)
{
    ...
    if(r != 0)
    {
        syslog(LOG_ERR,"DEV_[%s:%d] uv_ip4_addr error: %s",((CS104_Connection)self)->hostname,((CS104_Connection)self)->tcpPort,uv_strerror(r));
        return false;
    }
    ...
    if (r != 0)
    {
        syslog(LOG_ERR,"DEV_[%s:%d] uv_ip4_init error: %s",((CS104_Connection)self)->hostname,((CS104_Connection)self)->tcpPort,uv_strerror(r));
        return false;
    }
    //self->running=true;
    ...
    syslog(LOG_INFO,"DEV_[%s:%d] prepare uv_tcp_connect",((CS104_Connection)self)->hostname,((CS104_Connection)self)->tcpPort);
    r = uv_tcp_connect(connect, client,  (const struct sockaddr*) &addr, on_connect);
    if(r != 0)
    {
        syslog(LOG_ERR,"DEV_[%s:%d] uv_ip4_connect error: %s(%d)",((CS104_Connection)self)->hostname,((CS104_Connection)self)->tcpPort,uv_strerror(r),r);
        uv_close((uv_handle_t*)client, NULL);
        if(connect)
            free(connect);
        return false;
    }
    syslog(LOG_INFO,"DEV_[%s:%d] uv_ip4_connect ok(return 0)",((CS104_Connection)self)->hostname,((CS104_Connection)self)->tcpPort);
    self->running=true;
    return self->running;
    //return true;
}
